...
//    private List<Rule> content;
     ...
     */
    public SubGrammar(InputStream is, TrainedGrammar baseGrammar) {
...
//        Log.d("SG", "enter build sub grammar: " + content.toString());
        for (Rule r : content) {
//            Log.d("SG", "build sub grammar with rule: " + r.toString());
            if (r.lhs.startsWith("L"))
                continue;
            int freq = baseGrammar.getFrequency(r);
//            Log.d("test", "freq: " + freq);
...
//            for (Integer i: sizeFreqMap.keySet())
            for (Map.Entry<Integer, Integer> entry: sizeFreqMap.entrySet()) {
                if (entry.getKey() != ruleNum)
                    base += entry.getValue();
                else break;
            }
//            Log.d("test", "encode rule size base: " + base + " q: " + sizeTotalMap.get(nonT));
...
//            for (Integer i: sizeFreqMap.keySet())
//                if (p < sizeFreqMap.get(i))
//                    ruleNum = i;
//                else p -= sizeFreqMap.get(i);
            if (ruleNum == 0)
...
//        return new Pair<>((new SecureRandom()).nextInt(f) + l, q);
    }
