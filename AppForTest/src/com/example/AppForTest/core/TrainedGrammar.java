...
public class TrainedGrammar {
     ...
     */
    public TrainedGrammar(InputStream is) {
...
//                for (int i = 0; i < tRule.rhs.length() && !pt.isEmpty(); i++) {
//                    Rule r = pt.remove(0);
//                    tMap.put(r.lhs, r.rhs);
//                }
//                if (tMap.isEmpty()) break;
//                for (char c : tRule.rhs.toCharArray())
//                    sb.append(tMap.get("T_" + c));
            } else
                sb.append(pt.remove(0).rhs);
        }
        return sb.toString();
...
//        Log.d("TG", "find matched rules for " + s + "\n" + candidates.toString());
        Rule result = getMax(candidates);
        // TEST: print the max rule
//        Log.d("TG", "the max rule for " + s + " : " + ((result == null) ? "null" : result.toString()));
        return result;
    }
...
//        td = tm = ty = tY = null;
        ...
        double prob;
//        Map<String, Integer> dateList = G.get("T_d");
//        Map<String, Integer> monthList = G.get("T_m");
//        Map<String, Integer> yearList = G.get("T_y");
//        Map<String, Integer> YearList = G.get("T_Y");
        ...
        if (s.matches(dmy)){
            ...
            return null;
//        prob = G.get("T").get(rhs) / (double) totalFreq.get("T");
////                double prob = Math.pow(10, s.length() - 8);
////                prob *= timeList.get(rhs) / (double)frequencyMap.get("T");
//        if (td != null) {
//            pd = (double)(dateList.get(td) != null ? dateList.get(td) : 0) / totalFreq.get("T_d");
//            rules.add(new Rule("T_d", td, pd));
//            prob *= pd;
//        }
//        if (tm != null) {
//            pm = (double)(monthList.get(tm) != null ? monthList.get(tm) : 0) / totalFreq.get("T_m");
//            rules.add(new Rule("T_m", tm, pm));
//            prob *= pm;
//        }
//        if (ty != null) {
//            py = (double)(yearList.get(ty) != null ? yearList.get(ty) : 0) / totalFreq.get("T_y");
//            rules.add(new Rule("T_y", ty, py));
//            prob *= py;
//        }
//        if (tY != null) {
//            pY = (double)(G.get("T_Y").get(tY) != null ? G.get("T_Y").get(tY) : 0) / totalFreq.get("T_Y");
//            rules.add(new Rule("T_Y", tY, pY));
//            prob *= pY;
//        }
//        result = new Rule("T", rhs, prob);
//        result.setExtras(rules);
//        // TEST: print result of time string parsing
////        Log.d("TG", "result of parsing time: " + result.toString());
//        return result;
    }
...
//        Log.d("test", "encode rule " + lhs + " -> " + rhs + "\nbase+range+q: " + l + " " + f + " " + q);
...
//        Map<String, Integer> rhsMap = G.get(rule.lhs);
//        int newF;
//        if (rhsMap.containsKey(rule.rhs))
//            newF = rhsMap.get(rule.rhs) + 1;
//        else
//            newF = DEFAULT_FREQUENCY;
//        rhsMap.put(rule.rhs, newF);
//        totalFreq.put(rule.lhs, totalFreq.get(rule.lhs) + 1);
//        return newF;
    }
}
